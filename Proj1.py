    ...
    if userInput[0] == 'add' and len(userInput) > 1:
        if userInput[1] == 'item':
            ...
            if price != 0:
                ...
                newItem = myItem(vending.num_items + 1, userInput[2], int(userInput[3]), int(userInput[4]))
        ...
        if userInput[1] == 'item':
            ...
            if price!= 0:  #theoretically if the price != $0 then the item was found and the array value is returned
                checkFunds = calcPrice(price, myBalance)
                if checkFunds == -1:
        ...
        print(Fore.RED + "[add item name quantity price]\t\t", Fore.LIGHTRED_EX + "Adds an item to the vending machine if it does not already exist" + Style.RESET_ALL)
